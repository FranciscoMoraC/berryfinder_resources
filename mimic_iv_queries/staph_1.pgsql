...
create view inter_query_1 as 
	select microevent_id as event_id, subject_id, hadm_id, coalesce(charttime, chartdate) as date, ab_name, spec_type_desc, interpretation 
	from mimiciv_hosp.microbiologyevents m
	where interpretation is not null and interpretation <> 'P' and org_itemid = 80023 and hadm_id is not null;

SELECT * from inter_query_1 LIMIT 10;

-- Add patient demographic data, such as gender and age, by joining with the patients table
create view inter_query_2 as
	select iq.*, p.gender, p.anchor_age + extract(year from iq.date) - p.anchor_year as age
	from (select * from inter_query_1) as iq
	join (select * from mimiciv_hosp.patients) as p on iq.subject_id = p.subject_id;

SELECT * from inter_query_2 LIMIT 10;
...
create view inter_query_3 as 
	select iq.*, a.admission_type, a.admission_location, extract(day from iq.date - a.ADMITTIME) as admission_days,
		(a.deathtime is not null) as exitus, a.discharge_location, a.race, a.language
	from
		(select * from inter_query_2) as iq
	join
		(select * from mimiciv_hosp.admissions) as a on iq.hadm_id = a.hadm_id;

SELECT * from inter_query_3 LIMIT 10;
...
create view presc_view as 
	...
	join
		(select subject_id, hadm_id, ndc, stoptime, starttime from mimiciv_hosp.prescriptions) as pr
		on iq.subject_id = pr.subject_id AND iq.hadm_id = pr.hadm_id
	where iq.date > stoptime AND iq.date - INTERVAL '180 days' < pr.starttime and ndc <> '0'
...
select * from inter_query_4 LIMIT 10;
